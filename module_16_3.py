...
async def create_user(
    ...
    current_index = len(users.keys()) + 1
    users[current_index] = f'Имя: {username}, возраст: {age}'
    return f"User {current_index} is created!"
...
async def delete_user(user_id: Annotated[int, Path(ge=1, description="Enter User Id", example="75")]) -> str:
